...
│   │   └── 005_s13_trainer_invites.sql          # trainer_invites table, owner role constraint, RLS
│   ├── functions/
...
│   │   └── send-trainer-invite/         # Resend — trainer invite email with token link
│   └── email-templates/
...
| `/invite?token=xxx` | Trainer invite acceptance — validates token, creates trainer account |
...
| 12 | Three-mode theming system — dark (default) / light / high-contrast. New files: `src/hooks/useTheme.ts`, `src/contexts/ThemeContext.tsx`, `src/components/ui/ThemeToggle.tsx`. CSS: `index.css` now has full token blocks for all three themes under `[data-theme]` attribute selectors. `applyBrandTokens()` in `config/index.ts` refactored to only set client brand overrides, not text/semantic tokens (those are CSS-managed per mode). ThemeProvider wraps App, ThemeToggle in AppShell top bar. No-flash bootstrap script in `index.html` applies saved theme before React mounts. |
| 13 | Role-based trainer access control. Three roles: `owner` (manual DB set), `trainer` (invite-only), `student` (open self-reg). New: `src/pages/InvitePage.tsx` (/invite?token=), `src/components/owner/InviteManager.tsx`, `supabase/functions/send-trainer-invite/`, `supabase/migrations/005_s13_trainer_invites.sql`. Updated: `useAuth` (signUp locked to student, invite API), `authStore` (isOwner, owner inherits trainer rights), `ProtectedRoute` (owner passes trainer checks), `Register.tsx` (role selector removed), `AppShell` (owner labels), `Dashboard` (owner InviteManager panel), `types/index.ts` (UserRole += owner, TrainerInvite type). | — dark (default) / light / high-contrast. New files: `src/hooks/useTheme.ts`, `src/contexts/ThemeContext.tsx`, `src/components/ui/ThemeToggle.tsx`. CSS: `index.css` now has full token blocks for all three themes under `[data-theme]` attribute selectors. `applyBrandTokens()` in `config/index.ts` refactored to only set client brand overrides, not text/semantic tokens (those are CSS-managed per mode). ThemeProvider wraps App, ThemeToggle in AppShell top bar. No-flash bootstrap script in `index.html` applies saved theme before React mounts. |
...
Session 13 delivered the full role system. Three roles: `owner`, `trainer`, `student`.
- Register page locked to student-only
- `/invite?token=` page for trainer onboarding
- `send-trainer-invite` edge function (Resend email)
- `trainer_invites` table with RLS
- Owner sees InviteManager panel on dashboard

**First-deploy step:** After running migration 005, promote yourself to owner:
```sql
...
```bash
supabase functions deploy send-trainer-invite
supabase secrets set RESEND_API_KEY=re_xxxx APP_URL=https://yourdomain.com APP_NAME="Your Platform Name"
```
...
| Task | Priority | Notes |
|------|----------|-------|
| Cloudflare Worker (PayFast webhook) | High | `workers/payment-webhook/` is ready — just needs deploy + SEEN_PAYMENTS KV |
| Supabase Auth email templates | Medium | Templates in `supabase/email-templates/` — paste into Supabase Dashboard → Auth → Email Templates |
| DB Webhooks | Medium | Certificates issued → `send-certificate-email`; Payments complete → `send-payment-receipt` |
| PWA icons | Low | Add `icon-192.png` + `icon-512.png` to `public/icons/` |
