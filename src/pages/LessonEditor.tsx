...
export function LessonEditor() {
  ...
  useEffect(() => {
    ...
    if (courseId) {
      db.from('courses').select('title').eq('id', courseId).single()
        .then(({ data }: { data: any }) => { if (data) setCourseTitle(data.title) })
    }
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
      ...
      {/* ── Quiz shortcut (edit mode only, after lesson is saved) ── */}
      {isEdit && lessonId && (
        <div style={{
          ...
          <div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.78rem', color: 'var(--kw-cream)', marginBottom: '0.1rem' }}>
              Quiz
            </div>
            <div style={{ fontFamily: 'Syne Mono, monospace', fontSize: '0.62rem', color: 'var(--kw-muted)' }}>
              Add or edit the assessment for this lesson
            </div>
          </div>
          <Link
